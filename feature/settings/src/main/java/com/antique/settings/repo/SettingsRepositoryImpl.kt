...
class SettingsRepositoryImpl @Inject constructor(private val dispatcher: CoroutineDispatcher) : SettingsRepository {
    override suspend fun changeNickName(nickName: String): Boolean = withContext(dispatcher) {
        ...
        Firebase.database.reference.child(Constant.USER_NODE).child(uid).get().await().getValue(User::class.java)?.let {
