...
class WriteStoryViewModel @Inject constructor(
    ...
    private val _places = MutableLiveData<ApiState<Place>?>()
    val places: LiveData<ApiState<Place>?> get() = _places

    private val _registerStoryState = MutableLiveData<ApiState<Story>>()
    val registerStoryState: LiveData<ApiState<Story>> get() = _registerStoryState
    ...
    fun registerStory(body: String) {
        viewModelScope.launch {
            try {
                ...
                val place = place.value ?: com.antique.story.data.model.Place()
